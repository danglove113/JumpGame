	...
	void Update() 
	{
		...
		if(!isSpinning)
		{
			...
			if(slideChangeWithGestures && gestureListener)
			{
                ...
                {
                    if (controller.state == Controller.State.ShowList)
                    {
                        controller.Select(Controller.SwipeDirection.Left);
                    }
                    if (controller.state == Controller.State.Scan)
                    {
                        controller.Scan(-1);
                    }
                }
                ...
                else if (gestureListener.IsSwipeRight())
                {
                    if (controller.state == Controller.State.ShowList)
                    {
                        controller.Select(Controller.SwipeDirection.Right);
                    }
                    if (controller.state == Controller.State.Scan)
                    {
                        controller.Scan(1);
                ...
                else if (gestureListener.IsSwipeUp())
                {
                    ...
                    if (controller.state == Controller.State.ShowList)
                    {
                        controller.Select(Controller.SwipeDirection.Up);
                    }
                }
                ...
                else if (gestureListener.IsSwipeDown())
                {
                    ...
                    if (controller.state == Controller.State.ShowList)
                    {
                        controller.Select(Controller.SwipeDirection.Down);
                    }
                }
                ...
                else if (gestureListener.IsPush())
                {
                    print("push");
                    if (controller.state == Controller.State.Scan)
                        controller.state = Controller.State.ShowList;
                    else if (controller.state == Controller.State.ShowList)
                    {
                        controller.state = Controller.State.Scan;
                        controller.SetDepth();
                    }
                    controller.ChangeCanvas();
                }
